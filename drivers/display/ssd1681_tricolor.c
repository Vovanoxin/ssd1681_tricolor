...
static int ssd16xx_write_mono(const struct device *dev, const uint16_t x, const uint16_t y,
...
int ssd16xx_read_ram(const struct device *dev, enum ssd1681_ram ram_type, const uint16_t x,
    ...
    switch (ram_type) {
    case SSD1681_RAM_BLACK:
        ram_ctrl = SSD16XX_RAM_READ_CTRL_BLACK;
        break;

    case SSD1681_RAM_RED:
...
static int ssd16xx_read(const struct device *dev, const uint16_t x, const uint16_t y,
    ...
    return ssd16xx_read_ram(dev, SSD1681_RAM_BLACK, x, y, desc, buf);
}
...
static int ssd16xx_write(const struct device *dev, const uint16_t x, const uint16_t y,
    ...
    if (data->pixel_format == PIXEL_FORMAT_RGB_565) {
        ...
        return ssd16xx_write_mono(dev, x, y, desc, buf);
    }
}
...
static void ssd16xx_get_capabilities(const struct device *dev, struct display_capabilities *caps) {
    ...
    caps->supported_pixel_formats = PIXEL_FORMAT_MONO10 | PIXEL_FORMAT_RGB_565;
...
static int ssd16xx_clear_cntlr_mem(const struct device *dev, uint8_t ram_cmd) {
    ...
    if (config->profiles[SSD16XX_PROFILE_PARTIAL] == NULL && ram_cmd == SSD16XX_CMD_WRITE_RED_RAM) {
...
static int ssd16xx_set_profile(const struct device *dev, enum ssd16xx_profile_type type) {
    ...
    if (p && p->override_gate_line_width) {
        err = ssd16xx_write_uint8(dev, SSD16XX_CMD_GATE_LINE_WIDTH, p->override_gate_line_width);
        if (err < 0) {
            return err;
        }
    }
...
static int ssd16xx_controller_init(const struct device *dev) {
    ...
    data->blanking_on = false;
    data->profile = SSD16XX_PROFILE_INVALID; /* bits per plane = width*height bits, pack 1 bit per
                                                pixel → bytes = ceil(bits/8) */
    data->plane_bytes =
        (config->width * config->height + SSD16XX_PIXELS_PER_BYTE - 1) / SSD16XX_PIXELS_PER_BYTE;

    data->black_plane = k_malloc(data->plane_bytes);
    data->red_plane = k_malloc(data->plane_bytes);
    if (!data->black_plane || !data->red_plane) {
        LOG_ERR("Failed to allocate memory for display planes");
...
static int ssd16xx_init(const struct device *dev) {
    ...
    if (config->width > config->quirks->max_width || config->height > config->quirks->max_height) {
    ...
    static const struct ssd16xx_config ssd16xx_cfg_##n = {                                         \
        ...
        .profiles =                                                                                \
            {                                                                                      \
                ...
                [SSD16XX_PROFILE_PARTIAL] = SSD16XX_PROFILE_PTR(DT_CHILD(n, partial)),             \
            },                                                                                     \
    };                                                                                             \
                                                                                                   \
